...
class ApiClient {
  ...
  request = async (endpoint: string, options: RequestInit = {}) => {
    ...
    // If no token in localStorage, try to get a fresh session
    if (!token) {
      console.log('No token in localStorage, attempting to get fresh session...');
      try {
        ...
        if (session?.token) {
          ...
          console.log('Could not obtain fresh session');
        }
      } catch (error) {
        console.log('Could not get fresh session:', error);
      }
    }
    ...
    try {
      ...
      // Handle network errors (like connection refused, CORS, etc.)
      if (error instanceof TypeError) {
        console.error('Network error or CORS issue:', error);
        throw new Error('Network error: Unable to connect to the server. This may be due to a server issue or CORS policy.');
      }
  ...
  post = (endpoint: string, data: any) => {
    ...
    // Ensure difficulty_level is included if not already present
    if (!transformedData.difficulty_level) {
      transformedData.difficulty_level = 'intermediate';
    }
