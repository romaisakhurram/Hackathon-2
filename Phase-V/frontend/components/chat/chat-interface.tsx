...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const callChatAPI = async (message: string): Promise<Message> => {
    try {
      ...
      // If a task was created, updated, or deleted via chat, notify the parent component to refresh tasks
      if ((response.action === 'add_task' || response.action === 'update_task' || response.action === 'complete_task' || response.action === 'delete_task') && onTaskCreated) {
        onTaskCreated();
      }
            ...
            <div className="space-y-6">
              {messages.length === 0 && !isLoading && (
                <div className="flex flex-col items-center justify-center h-full text-center py-12">
                  ...
                  <h3 className="text-xl font-semibold mb-2">Todo AI Assistant</h3>
