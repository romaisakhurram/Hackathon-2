...
def init_engines():
    ...
    if DATABASE_URL.startswith("postgresql"):
        ...
        # Try to use PostgreSQL with proper parameters for asyncpg (async driver)
        try:
            # For async operations, we need to use asyncpg driver
            ...
            # For sync operations, we can use psycopg2
            sync_db_url = DATABASE_URL  # Use original URL for sync operations

            print(f"Async DB URL: {async_db_url[:50]}...")  # Debug: print first 50 chars

            # Handle SSL parameters for asyncpg - remove sslmode from URL and pass separately
            import urllib.parse
            parsed_url = urllib.parse.urlparse(async_db_url)
            ...
            if 'sslmode' in query_params:
                ...
                # Map sslmode to asyncpg ssl parameters
                if ssl_value == 'require':
                    ssl_params['ssl'] = True
                elif ssl_value == 'disable':
                    ssl_params['ssl'] = False
                # Remove sslmode from query params since asyncpg doesn't accept it directly
                del query_params['sslmode']

            # Reconstruct URL without sslmode
            new_query = urllib.parse.urlencode(query_params, doseq=True)
            async_db_url_clean = urllib.parse.urlunparse((
                parsed_url.scheme,
                parsed_url.netloc,
                parsed_url.path,
                parsed_url.params,
            ...
            async_engine = create_async_engine(
                async_db_url_clean,  # URL with asyncpg driver for async operations (without sslmode)
                echo=False,
                pool_size=5,
                max_overflow=10,
            ...
            sync_engine = create_engine(
                sync_db_url,  # Use original URL for sync operations
                echo=False,
                pool_size=5,
                max_overflow=10,
                pool_pre_ping=True,
            ...
            print("Using PostgreSQL database with asyncpg/psycopg2 drivers")
...
# FastAPI dependency function that works with Depends()
async def get_async_session() -> AsyncSession:
    """
    Get an async database session as a FastAPI dependency.
    This function can be used with Depends() in FastAPI routes.
    """
    global async_engine
...
async def get_async_session_context() -> AsyncGenerator[AsyncSession, None]:
    ...
    This is a proper async context manager that can be used with 'async with'.
    """
    global async_engine
